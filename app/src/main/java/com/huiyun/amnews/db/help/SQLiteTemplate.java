package com.huiyun.amnews.db.help;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

import com.huiyun.amnews.db.manager.DBManager;
...
//	protected String customerKey = "phone_cust";
//	protected String subsKey = "subscription_id";
	...
	public synchronized int updateCustoemrById(String table, String id, ContentValues values) {
		try {
			dataBase = dBManager.openDatabase();
			return dataBase.update(table, values, mPrimaryKey + "=?",
					new String[] { id });
	...
	public synchronized int updateSubsById(String table, String id, ContentValues values) {
		try {
			dataBase = dBManager.openDatabase();
			return dataBase.update(table, values, mPrimaryKey + "=?",
					new String[] { id });
	...
	public Map<String, String> queryEngagementIdAndLastMotifiedTimeForMap(String sql,
			String[] selectionArgs) {
		Cursor cursor = null;
		Map<String, String> engagementMap = null;
		try {
			dataBase = dBManager.openDatabase();
			cursor = dataBase.rawQuery(sql, selectionArgs);
			
			engagementMap = new HashMap<String, String>();
			while (cursor.moveToNext()) {
				String id = cursor.getString(cursor.getColumnIndex("id"));
				String last_modified_time = cursor.getString(cursor.getColumnIndex("last_modified_time"));
				engagementMap.put(id, last_modified_time);
			}
		} finally {
			if (!isTransaction) {
				closeDatabase(cursor);
